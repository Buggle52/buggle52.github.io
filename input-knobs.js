window.addEventListener("load",()=>{
  ...
  //Added
  op.angleSweep = op.angleSweep||270;
  op.angleStart = op.angleStart||-135;
  //
...
input[type=range].input-knob{
  width:${op.knobWidth}px; _height:${op.knobHeight}px;
}
...
<circle fill="${fg}" cx="${w*0.5}" cy="${h*1.5}" r="${minwh*0.25}"/></g></svg>`;
  ...
  let initKnobs=(el)=>{
    ...
    el.redraw=(f)=>{
      astart=parseFloat(el.getAttribute("angle-start")||op.angleStart);
      asweep=parseFloat(el.getAttribute("angle-sweep")||op.angleSweep);
      ...
      if(f||ik.valueold!=el.value){
        ...
        if(ik.sprites>=1)
          ...
          case "k":
            ...
            el.style.transform="rotate("+(asweep*v+astart)+"deg)";
            break;
          case "h":
